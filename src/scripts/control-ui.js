...
function initUI(flock) {
  ...
  /**
     ...
     */
  const defaultSeparationCoefficient = 1e-2 * 5;
  const defaultCohereCoefficient = 1e-2 * 5;
  const defaultAlignCoefficient = 1e-2 * 5;
  /**
     * Initialize separate slider.
     */
  const separationCoefficientSlider = document.getElementById("separation-range");
  const separationOutput = document.getElementById("separation-value");
  separationOutput.innerHTML = separationCoefficientSlider.value;

  /**
     * Initialize cohere slider.
     */
  const cohereCoefficientSlider = document.getElementById("cohere-range");
  const cohereOutput = document.getElementById("cohere-value");
  cohereOutput.innerHTML = cohereCoefficientSlider.value;

  /**
     * Initialize align slider.
     */
  const alignCoefficientSlider = document.getElementById("align-range");
  const alignOutput = document.getElementById("align-value");
  ...
  const fovOutput = document.getElementById("fov-value");
  ...
  separationCoefficientSlider.addEventListener("input", () => {
    const separationSliderValue = defaultSeparationCoefficient * separationCoefficientSlider.value;
    flock.forEach((boid) => {
  ...
  cohereCoefficientSlider.addEventListener("input", () => {
    const cohereSliderValue = defaultCohereCoefficient * cohereCoefficientSlider.value;
    flock.forEach((boid) => {
  ...
  alignCoefficientSlider.addEventListener("input", () => {
    const alignSliderValue = defaultAlignCoefficient * alignCoefficientSlider.value;
    flock.forEach((boid) => {
  ...
  separateSwitch.onclick = function () {
    ...
    if (!separateSwitch.checked) {
      const steerElementScale = new Vector2D(0, 0);
