...
class Boid {
  ...
  constructor({ id, isHighlighted, world }) {
    this.id = id;
    ...
    this.position = new Vector2D(Math.random()
  ...
  repel(deltaT) {
    const offset = 20;
    const { width, height } = DEFAULT_BOID_WORLD;
    const bottomWall = height - offset;
    const rightWall = width - offset;
    const topWall = offset;
    const leftWall = offset;
    const repelCoefficient = 0.5;

    const totalSteer = new Vector2D(0, 0);
  ...
  /**
     ...
     */
  superGravity(deltaT) {
    const { height, width } = DEFAULT_BOID_WORLD;
    const offset = -10;
    ...
    const desiredVelocity = new Vector2D(width / 2, height / 2);
  ...
  repelSteer(steerDirectionX, steerDirectionY, distance) {
    ...
    const strengthRatio = (1 - (distance / this.range));
  ...
  toggleFOV() {
    if (!this.FOVEnabled) {
      ...
      // Percentage of FOV that is not blind
      const viewPercentage = ((2 * this.leftSideFOV / (Math.PI / 180)) / 360) * 100;
  ...
  /**
     ...
     */
  speedControl() {
    const newSpeed = this.velocity.normalize();
  ...
  separate(deltaT) {
    ...
    this.neighbors.forEach((neighborBoid) => {
      const desiredVelocity = new Vector2D(this.position.x, this.position.y);
      ...
      const strengthRatio = (1 - (distance / this.range)) ** 2;
    ...
    if (this.showSeparate) {
      const steerElementScale = new Vector2D(totalSteer.x, totalSteer.y);
  ...
  align(deltaT) {
    ...
    let totalSteer = new Vector2D(0, 0);
    const desiredVelocity = new Vector2D(0, 0);
    ...
    if (this.showAlign) {
      const steerElementScale = new Vector2D(totalSteer.x, totalSteer.y).scale(2000);
  ...
  cohere(deltaT) {
    ...
    if (this.neighbors.size > 0) {
      ...
      const desiredVelocity = new Vector2D(centerOfMass.x, centerOfMass.y);
      desiredVelocity.subtract(this.position);
      const normalizedSteer = this.steerTowardsTarget(desiredVelocity);
    ...
    if (this.showCohere) {
      const steerElementScale = new Vector2D(totalSteer.x, totalSteer.y).scale(300);
  ...
  steerTowardsTarget(desiredDirection) {
    ...
    const desiredSteer = new Vector2D(desiredVelocity.x, desiredVelocity.y);
  ...
  drawLine(lineElement, vector, styles = {}) {
    ...
    const tempVector = new Vector2D(vector.x, vector.y);
